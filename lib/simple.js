	...
	var sock = Object.defineProperties(conn, {
		...
		sendFile: {
			...
			async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
				...
				if (file.res !== null) {
					mimetype = options.mimetype || (!file.nomime ? file.mime : file.res.headers.get('content-type'));
				} else {
					mimetype = options.mimetype || file.mime
				}
...
export function serialize() {
	return Object.defineProperties(proto.WebMessageInfo.prototype, {
		...
		isBaileys: {
			get() {
				return this.id?.length === 16 || this.id?.startsWith('SKIZO') || this.id?.startsWith('3EB0') && this.id?.length === 12 || false
			}
		},
		chat: {
			get() {
...
export function protoType() {
	...
	Buffer.prototype.Webp2Png = async function Webp2Png() {
		var c = await fileTypeFromBuffer(this)
		if (c.ext == 'webp') {
			return getbuffer(await webp2png(this))
		}
	}
	/**
	 * @returns {Buffer}
