...
var logger = P({
	...
	level: Helper.opts['no-scan'] || Helper.opts['pairing'] || Helper.opts['pair'] ? 'silent' : 'info'
}, stream).child({
	class: 'baileys'
})
...
var connectionOptions = {
	printQRInTerminal: Helper.opts['no-scan'] || Helper.opts['pairing'] || Helper.opts['pair'] ? false : true,
...
async function start(oldSocket = null, opts = {
	...
	var conn = makeWASocket({
		...
		browser: Browsers.ubuntu('Chrome'), //['Chrome (Linux)', '', ''], //browser: ['@findme-19 / milkita-bot', 'safari', '4.0.0'],
	...
	console.clear()
	if (Helper.opts['no-scan'] || Helper.opts['pairing'] || Helper.opts['pair'] && !conn.authState.creds.me) {
		//var inq = await inquery.prompt(askForNumber)
		delay(5000).then(async (v) => {
			if (!nomorbot.replace(/[^0-9]/g, "")) {
				console.log('phone number not valid')
				process.kill(process.pid, 'SIGKILL')
			}
			console.log('phone number: ' + nomorbot.replace(/[^0-9]/g, ""))
			console.log('use this code in your whatsapp: ' + await conn.requestPairingCode(nomorbot.replace(/[^0-9]/g, "")))
		})
	}
